...
// cli.js/config.js/values.js use an environment var named `DOC_VERSION` to limit the build to
// a single version. We will use the same in `docusaurus.config.js`:
var envVersion = [];
if (process.env.DOC_VERSION && process.env.DOC_VERSION !== "all") {
  envVersion.push(process.env.DOC_VERSION);
    console.log("env DOC_VERSION is set to: " + process.env.DOC_VERSION);
}

// Give guidance if the old `BUILD_FAST` is used:
const isBuildFast = !!process.env.BUILD_FAST;
if (isBuildFast){
    console.log("Please unset BUILD_FAST and set `DOC_VERSION` to the desired version.\nFor example, `unset BUILD_FAST && export DOC_VERSION=3.3`");
    process.exit(1);
}
